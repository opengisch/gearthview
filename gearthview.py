...
def startGeoDrink_Server(self):
    ...
    # 	os.system("open " + str(webServerDir + 
    # 'QGIS_link.kmz'))

    # if platform.system() == "Linux":            
    # 	os.system("xdg-open " + str(webServerDir + 
    # 'QGIS_link.kmz'))
    ...
    if (serverStarted == 0):
        ...
        class FormPage(Resource):
            ...
            def render_GET(self, request):
                ...
                # <GET /form?BBOX=16.3013171267662,
                # 38.63325421913416,16.62443680433362,
                # 38.86443091553171 HTTP/1.1>
                # <GET /form?p=1&BBOX=-0.02411607307235109,
                # -0.08678435516867355,0.149613182683106,
                # 0.06773426441872454 HTTP/1.1>
                # <GET 
                # /form?p=0&bboxWest=12.13495518195707&%0A%09%09
                # %09&bboxSouth=42.72418498839377&%0A%09%09%09
                # &bboxEast=12.13932970664519&%0A%09%09%09&bboxNorth=42
                # .72713741547008&%09%0A%09%09%09&lookatTerrainLon=12
                # .13714248521776&%0A%09%09%09&lookatTerrainLat=42
                # .7256612227012&%0A%09%09%09&lookatTerrainAlt=112.88& 
                # HTTP/1.1>
                # <GET /form?p=0&BBOX=12.1356167437372,
                # 42.7264889613262,12.13701876665762,
                # 42.72743519322518&LookAt=12.13631776054357,
                # 42.72696207941286,
                # 205.39&LookatHeading=-0.002&LookatTilt=0&LookatTerrain=12
                # .13631776055484,42.72696207945101,117.08&terrain=1 HTTP/1.1>
                # <GET /form?p=0&BBOX=12.13560526927239,
                # 42.72643977188249,12.13703061697471,
                # 42.72740174642745&LookatTerrain=12.13631794864158,
                # 42.72692076136277,116.9&terrain=1 HTTP/1.1>
                # <GET /form?p=1&BBOX=10.20045469843596,
                # 43.66302191930933,10.66672168693798,
                # 43.8667284090175&LookatTerrain=10.43397243010223,
                # 43.76510664670415,
                # 13.83&terrain=1&CAMERA=10.43397248408143,
                # 43.76510665598479,29287.22,0,-0.556&VIEW=60,38.141,1306,
                # 782 HTTP/1.1>

                # <viewFormat>
                #            BBOX=[bboxWest],[bboxSouth],[bboxEast],
                # [bboxNorth];
                #            LookatTerrain=[lookatTerrainLon],
                ...
                msg = ("LonLatH = %s, %s    elev = %s m      alt =  %s m") % (
                lon, lat, Zeta, lookatRange)
                ...
                if (pony != '0'):
                    ...
                    qrCodeUrl = (
                                "http://qrcode.kaywa.com/img.php?s=8&amp;d=%.14f,%.14f,%.2f") % (
                                lat, lon, Zeta)

                    descript = (
                               '<html>lat  long  H<br><br>%.14f,%.14f,'
                               '%.2f<br><br><table border=1 '
                               'style="border-collapse:collapse; '
                               'border-color:#000000;"cellpadding=0 '
                               'cellspacing=0  width=250 style="FONT-SIZE: '
                               '11px; FONT-FAMILY: Verdana, Arial, '
                               'Helvetica, sans-serif;"><tr><td '
                               'bgcolor="#E3E1CA" align="right"><font '
                               'COLOR="#FF0000"><b>CODE</b></font></td><td '
                               'bgcolor="#E4E6CA"> <font COLOR="#008000">') % (
                               lat, lon, Zeta)
                    qrCodeImg = ('<img alt="" src="%s" /></html>') % (
                    qrCodeUrl)
            ...
            def render_POST(self, request):
                ...
                # self.iface.mainWindow(), "GEarthViewServer", u"recieved 
                # something.\n{}".format(newdata))

                if newdata['bbox']:
                    self.iface.mapCanvas().setExtent(
                        QgsRectangle(newdata['bbox']['xmin'],
                                     newdata['bbox']['ymin'],
                                     newdata['bbox']['xmax'],
                                     newdata['bbox']['ymax']))
                else:
...
def QGEarth_addPoint(self):
    if (serverStarted == 0):
        ...
        #  "You need to startQrCoding, before !!!", "")
        self.iface.messageBar().pushMessage("WARNING",
                                            "You need to startQrCoding, "
                                            "before !!!",
    ...
    if layer:
        if layer.type() == layer.VectorLayer:
            ...
            if (Edit == 0):
                ...
                # self.iface.mainWindow(), "WARNING: Layer not editable", 
                # str(layer.name()))
                self.iface.messageBar().pushMessage("WARNING",
                                                    "Layer not editable",
                                                    level=QgsMessageBar.WARNING,
        ...
        geomType = "Point" + '?crs=proj4:' + \
                   QgsProject.instance().readEntry("SpatialRefSys",
                                                   "/ProjectCRSProj4String")[0]
...
def GDX_Publisher(self):
    ...
    tempdir = unicode(QFileInfo(
        QgsApplication.qgisUserDbFilePath()).path()) + \
              "/python/plugins/gearthview/temp"
    ...
    # Prendo le coordinate della finestra 
    # attuale---------------------------------------
    # 13.5702225179249876,41.2134192420407501 : 13.5768356834183166,
    # 41.2182110366311107
    text = mapCanvas.extent().toString()
    text1 = text.replace(",", " ")
    text2 = text1.replace(" : ", ",")
    # the_filter = "bbox($geometry, geomFromWKT ( 'LINESTRING(" 
    # + text2 + ")'))"
    ...
    if QGis.QGIS_VERSION_INT <= 120200:
        ...
        mapSettings.setFlags(
            QgsMapSettings.Antialiasing | QgsMapSettings.UseAdvancedEffects 
    ...
    crsSrc = srs  # QgsCoordinateReferenceSystem(layer.crs())   # prendere 
    # quello attuale
    ...
    loc = (
          "    	 <description><![CDATA["
          "http://map.project-osrm.org/?loc=%.9lf,"
          "%.9lf&ly=1784084387]]></description>\n") % (
          yc, xc)

    kml.write(loc)
    #
    # kml.write('    	 <description>http://map.project-osrm.org
    # /?hl=it&loc=45.989486,12.778154&loc=45.985624,
    # 12.781076&z=16&center=45.984058,
    # 12.774417&alt=0&df=0&re=0&ly=-940622518</description>\n')
    ...
    stringazza = (
                 "%.9lf,%.9lf,0 %.9lf,%.9lf,0 %.9lf,%.9lf,0 %.9lf,%.9lf,"
                 "0\n") % (
        ...
        if layer.type() == layer.VectorLayer:
            ...
            for feat in iter:
                ...
                if geom.type() == QGis.Point:
                    ...
                    # h ttp://qgis.spatialthoughts.com/2012/11/tip-count
                    # -number-of-vertices-in-layer.html				        
                    if geom.isMultipart():
                        print "MULTIPART !!!"
...
def GDX_Publisher2(self):
    ...
    # /placemark_circle_highlight.png</href>\n')
    # kml = kml + ('	     			</Icon>\n')
    # kml = kml + ('	     		</IconStyle>\n')
    # kml = kml + ('	     		<LabelStyle>\n')
    # kml = kml + ('	     			<scale>0.7</scale>\n')
    ...
    # stringazza =    ("%.9lf,%.9lf,0 %.9lf,%.9lf,0 %.9lf,
    # %.9lf,0 %.9lf,%.9lf,0\n") % (x1, y1, x2, y2, x3, y3, x4, y4)        		
    # kml = kml + (stringazza)				
    ...
    # f = open(out_folder + "/" + nomePNG	+ ".pngw", 
    # 'w')				
    # f.write(str(xScale) + '\n')
    # f.write(str(0) + '\n')
    # f.write(str(0) + '\n')
    ...
    if layer:
        ...
        # kmlFile = codecs.open(out_folder + '/doc.kml', 'w', 
        # encoding='utf-8')
        # kmlFile.write(kml)			
        # kmlFile.close()
...
class gearthview:
    ...
    def initGui(self):
        ...
        self.action = QAction(QIcon(self.plugin_dir + "/iconG.png"),
                              QCoreApplication.translate(u"GEarthView",
                                                         "GEarthView"),
        ...
        self.PasteFromGEaction = QAction(
            QIcon(self.plugin_dir + "/iconQG.png"),
            QCoreApplication.translate(u"GEarthView", "PasteFromGE"),
            self.iface.mainWindow())
        QObject.connect(self.PasteFromGEaction, SIGNAL("triggered()"),
                        self.PasteFromGE)

        #        self.QRcodingAction= QAction(QIcon(self.plugin_dir + 
        # "/iconQR.png"), QCoreApplication.translate(u"&GEarthView", 
        # "GE_QRcoding"), self.iface.mainWindow())
        #        QObject.connect(self.QRcodingAction, SIGNAL("triggered()"),
        #  self.startQrCoding)

        #        self.QGEarthAction= QAction(QIcon(self.plugin_dir + 
        # "/iconQG.png"), QCoreApplication.translate(u"&GEarthView", 
        # "PointFromGE"), self.iface.mainWindow())
        #        QObject.connect(self.QGEarthAction, SIGNAL("triggered()"), 
        # self.QGEarth)        

        self.aboutAction = QAction(QIcon(self.plugin_dir + "/iconA.png"),
                                   QCoreApplication.translate(u"&GEarthView",
                                                              "About"),
                                   self.iface.mainWindow())
        ...
        #        self.iface.addPluginToWebMenu(u"&GEarthView", 
        # self.QRcodingAction)
    ...
    def unload(self):
        ...
        # virtual void QgisInterface::removeToolBarIcon 	( 	QAction *  	
        # qAction	) 	
    ...
    def PasteFromGE(self):
        ...
        for iLayer in range(mapCanvas.layerCount()):
            ...
            if (layer.name() == "GEKml_Points") or (
                layer.name() == "GEKml_Lines") or (
                layer.name() == "GEKml_Polygons"):
        ...
        if (GEKml_Polygons > 0):
            salvalo2 = codecs.open(tempdir + "/GEKml_Polygons.kml", 'w',
                                   encoding='utf-8')
            salvalo2.write(copyText)

            salvalo2.close()

            vlayer = QgsVectorLayer(tempdir + "/GEKml_Polygons.kml",
                                    "GEKml_Polygons", "ogr")
            QgsMapLayerRegistry.instance().addMapLayer(vlayer)

        if (GEKml_Lines > 0):
            salvalo2 = codecs.open(tempdir + "/GEKml_Lines.kml", 'w',
                                   encoding='utf-8')
            salvalo2.write(copyText)

            salvalo2.close()

            vlayer = QgsVectorLayer(tempdir + "/GEKml_Lines.kml",
                                    "GEKml_Lines", "ogr")
            QgsMapLayerRegistry.instance().addMapLayer(vlayer)

        if (GEKml_Points > 0):
            salvalo2 = codecs.open(tempdir + "/GEKml_Points.kml", 'w',
                                   encoding='utf-8')
            salvalo2.write(copyText)

            salvalo2.close()

            vlayer = QgsVectorLayer(tempdir + "/GEKml_Points.kml",
                                    "GEKml_Points", "ogr")
            QgsMapLayerRegistry.instance().addMapLayer(vlayer)
    ...
    def about(self):
        infoString = QCoreApplication.translate('GEarthView',
                                                "GEarthView Plugin<br>This plugin displays QGis view into Google Earth.<br>Author:  Bob MaX (aka: geodrinx)<br>Mail: <a href=\"mailto:geodrinx@gmail.com\">geodrinx@gmail.com</a><br>Web: <a href=\"http://exporttocanoma.blogspot.it\">exporttocanoma.blogspot.it</a><br></b>.")
        ...
        QMessageBox.information(self.iface.mainWindow(),
                                "About GEarthView plugin", infoString)
...
#        self.iface.removeToolBarIcon(self.action)
    ...
    def run(self):
        ...
        webServerDir = unicode(QFileInfo(
            QgsApplication.qgisUserDbFilePath()).path()) + "/python/plugins/gearthview/_WebServer/"
